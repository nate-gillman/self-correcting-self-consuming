...
class MDM(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        """
        bs, njoints, nfeats, nframes = x.shape  # [1, 263, 1, 196]
        emb = self.embed_timestep(timesteps)  # [1, bs, d] = [1, 1, 512]

        force_mask = y.get("uncond", False)
        if "text" in self.cond_mode:
            enc_text = self.encode_text(y["text"])
            try:
                emb += self.embed_text(self.mask_cond(enc_text, force_mask=force_mask).type(torch.float32))
            except:
                # emb = emb + self.embed_text(self.mask_cond(enc_text, force_mask=force_mask).type(torch.float64))
                emb += self.embed_text(self.mask_cond(enc_text, force_mask=force_mask).type(torch.float64))
        ...
        x = self.input_process(x)  # [1, 263, 1, 196]; embedding for every frame

        if self.arch == "trans_enc":  # transformer encoder
            # adding the timestep embed
            xseq = torch.cat((emb, x), axis=0)  # [seqlen+1, bs, d] = [197, 1, 512]
            xseq = self.sequence_pos_encoder(
                xseq
            )  # [seqlen+1, bs, d] = [197, 1, 512]. adds sins/cosines
            output = self.seqTransEncoder(xseq)[
                1:
            ]  # [seqlen, bs, d]   = [196, 1, 512]. src_key_padding_mask=~maskseq)  #
...
class TimestepEmbedder(nn.Module):
    def __init__(self, latent_dim, sequence_pos_encoder) -> None:
        super().__init__()
        self.latent_dim = latent_dim
...
class InputProcess(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.data_rep in ["rot6d", "xyz", "hml_vec"]:
            try:
                x = self.poseEmbedding(x.type(torch.float32))  # [seqlen, bs, d]
            except:
                x = self.poseEmbedding(x.type(torch.float64))
            return x
...
class OutputProcess(nn.Module):
    ...
    def forward(self, output):
        ...
        # print("TO DO: figure out data rep, for the different datasets... this one uses hml_vec... what is that???")
        if self.data_rep in ["rot6d", "xyz", "hml_vec"]:
