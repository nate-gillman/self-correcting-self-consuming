...
try:
    ...
    from UniversalHumanoidControl.uhc.utils.transform_utils import convert_orth_6d_to_aa
    from UniversalHumanoidControl.uhc.smpllib.smpl_mujoco import smpl_to_qpose
...
class DatasetAMASSSingle:
    ...
    def __init__(self, data_specs, data_mode="train"):
        ...
        # self.pickle_data = joblib.load(open("UniversalHumanoidControl/" + self.data_root, "rb"))
        # for ^ that one, the key is the name of the motion, and the nested keys are dict_keys(['pose_aa', 'pose_6d', 'trans', 'beta', 'seq_name', 'gender'])
        self.pickle_data = data_specs["preprocessed_motion"]

        self.mode = data_specs.get("mode", "all")
        self.adaptive_iter = data_specs.get("adaptive_iter", -1)
        # self.netural_path = data_specs.get("neutral_path", "UniversalHumanoidControl/sample_data/standing_neutral.pkl")
        self.netural_path = "UniversalHumanoidControl/sample_data/standing_neutral.pkl"
    ...
    def process_data_pickle(self, pk_data):
        ...
        for k in tqdm(process_keys):
            ...
            if data_out["beta"][k].shape[1] != 16:
                data_out["beta"][k] = np.concatenate([data_out["beta"][k], np.zeros((seq_len, 16 - v["beta"].shape[0]))], axis=1)
