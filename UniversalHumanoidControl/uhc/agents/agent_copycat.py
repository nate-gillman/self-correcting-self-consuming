...
try:
    ...
    from UniversalHumanoidControl.uhc.khrylib.utils import to_device, create_logger, ZFilter, get_eta_str
    from UniversalHumanoidControl.uhc.khrylib.rl.core import LoggerRL
    from UniversalHumanoidControl.uhc.khrylib.utils.memory import Memory
    from UniversalHumanoidControl.uhc.khrylib.utils.torch import *
    from UniversalHumanoidControl.uhc.khrylib.rl.core import estimate_advantages
    from UniversalHumanoidControl.uhc.khrylib.rl.agents import AgentPPO
    ...
    from UniversalHumanoidControl.uhc.losses.reward_function import reward_func
    from UniversalHumanoidControl.uhc.models.policy_mcp import PolicyMCP
    from UniversalHumanoidControl.uhc.khrylib.models.mlp import MLP
    from UniversalHumanoidControl.uhc.envs.humanoid_im import HumanoidEnv
    from UniversalHumanoidControl.uhc.data_loaders.dataset_amass_single import DatasetAMASSSingle
    from UniversalHumanoidControl.uhc.smpllib.smpl_parser import SMPL_BONE_ORDER_NAMES
    from UniversalHumanoidControl.uhc.smpllib.smpl_eval import compute_metrics
    from UniversalHumanoidControl.uhc.utils.flags import flags
    from UniversalHumanoidControl.uhc.utils.tools import CustomUnpickler
...
class AgentCopycat(AgentPPO):
    ...
    def load_checkpoint(self, iter):
        ...
        if iter > 0:
            ...
            cp_path = osp.join("UniversalHumanoidControl", cp_path)
            self.logger.info("loading model from checkpoint: %s" % cp_path)
            model_cp = CustomUnpickler(open(cp_path, "rb")).load()
            self.policy_net.load_state_dict(model_cp["policy_dict"])
            self.value_net.load_state_dict(model_cp["value_dict"])
        ...
        for data_loader in data_loaders:
            ...
            with to_cpu(*self.sample_modules):
                with torch.no_grad():
                    ...
                    print(f"Running UHC eval (multiprocessing) with {len(jobs)} jobs...")
                ...
                if self.data_loader.data_specs["preprocessed_motion"]: # i.e. if we hacked code to work around pkls
                    res_dicts.append({
                        "imitation_of_synthetic_motions" : data_res_coverage,
                    })
                else:
                    res_dicts.append({
                        f"coverage_{data_loader.name}": data_res_metrics,
                        f"coverage_{data_loader.name}_motions": res,
                    })
        ...
        with to_cpu(*self.sample_modules):
            with torch.no_grad():
                ...
                for t in range(10000):
                    ...
                    # gt is is 76 dimensional. the last 72 correspond to the 24 joints...
                    res["gt"].append(self.env.get_expert_attr("qpos", self.env.get_expert_index(t)).copy())
                    res["gt_pose_aa"].append(self.env.get_expert_attr("pose_aa", self.env.get_expert_index(t)).copy()) # this is the ground truth amass signal...
                    # print(self.env.get_expert_attr("qpos", self.env.get_expert_index(t)).copy(), "\n")
                    res["pred"].append(self.env.data.qpos.copy())
                    res["pred_body_quat"].append(self.env.get_body_quat().copy())

                    # gt_jpos is (24*3) dimensional. I wrote down the relevant kinematic chain at some point...
                    # 2 are hands. this is 3D coordinates of joints. so this is the one for visualization
                    ...
                    if done:
                        ...
                        if self.cfg.fail_safe and info["percent"] != 1:
                            ...
                            # I np.saved this... see instructions in MDM readme

                            save_motion_to_mdm_repo = False
                            if save_motion_to_mdm_repo:
                                out_dir = os.path.join("/oscar/data/csun45/nates_stuff/UniversalHumanoidControl", take_key, self.cfg.cfg)
                                os.makedirs(out_dir, exist_ok=True)
                                out_file = os.path.join(out_dir, "motion.npy")
                            ...
                            if visualize_motion:
                                ...
                                visualize_samples_and_save_to_disk(
                                    res["pred_jpos"], # res["gt_jpos"],
                                    "/oscar/data/csun45/nates_stuff/UniversalHumanoidControl",
                                    "gt.mp4",
                                    20
                                )
