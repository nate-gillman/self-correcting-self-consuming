...
try:
    ...
    from UniversalHumanoidControl.uhc.khrylib.utils import recreate_dirs
    from UniversalHumanoidControl.uhc.utils.config_utils.base_config import Base_Config
...
class Config(Base_Config):
    ...
    def find_asset(self, asset_path):
        if not path.exists(asset_path):
            fullpath = path.join(
                self.base_dir, "UniversalHumanoidControl/assets/mujoco_models", path.basename(asset_path)
            )
