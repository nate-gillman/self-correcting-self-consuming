...
try:
    ...
    from translation.utils_mdm_to_amass.human_body_prior.src.human_body_prior.models.model_components import BatchFlatten
    from translation.utils_mdm_to_amass.human_body_prior.src.human_body_prior.tools.rotation_tools import matrot2aa
...
class VPoser(nn.Module):
    ...
    def encode(self, pose_body):
        ...
        '''
        try:
            res = self.encoder_net(pose_body)
        except:
            try:
                res = self.encoder_net(pose_body.to(torch.float32))
            except:
                res = self.encoder_net(pose_body.to(torch.float64))
