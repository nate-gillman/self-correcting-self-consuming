...
try:
    ...
    from translation.utils_mdm.body_model import BodyModel
    from translation.utils_mdm.skeleton import Skeleton
    from translation.utils_mdm.quaternion import qbetween_np, qrot, qrot_np, qmul_np, qinv, \
        qinv_np, qfix, quaternion_to_cont6d, quaternion_to_cont6d_np
    from translation.utils_mdm.paramUtil import t2m_raw_offsets, t2m_kinematic_chain
...
# TO DO... change these path...
male_bm_path = 'body_models/smplh/female/model.npz'
...
def recover_root_rot_pos(data):
    ...
    r_rot_quat = torch.zeros(data.shape[:-1] + (4,), dtype=torch.float32).to(data.device)
    r_rot_quat[..., 0] = torch.cos(r_rot_ang)
    r_rot_quat[..., 2] = torch.sin(r_rot_ang)

    r_pos = torch.zeros(data.shape[:-1] + (3,), dtype=torch.float32).to(data.device)
...
if __name__ == "__main__":
    ...
    (   
        pose_seq,   # 263-dimensional;input and output from the MDM model
        pose_seq_3D # (22,3)-dimensiona and represents 3D cartesian coordinates
    ...
    save_path = 'translation/data/mdm/{}.npy'.format(action)
