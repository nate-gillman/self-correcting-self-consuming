...
#from mdm_to_amass import offset_position

# ========== Configuration and Environment Setup ==========

# os.environ['PYOPENGL_PLATFORM'] = 'egl'
male_bm_path = './translation/utils_mdm/body_models/smplh/male/model.npz'
male_dmpl_path = './translation/utils_mdm/body_models/dmpls/male/model.npz'
female_bm_path = './translation/utils_mdm/body_models/smplh/female/model.npz'
female_dmpl_path = './translation/utils_mdm/body_models/dmpls/female/model.npz'
...
def raw_pose_processing(amass_raw):
    ...
    fps = 20 #bdata['mocap_framerate']
    print('poses shape ', bdata['poses'].shape[0])
    print('trans shape ', bdata['trans'].shape[0])
    ...
    with torch.no_grad():
        for fId in range(0, frame_number, down_sample):
            ...
            pose_hand = torch.Tensor(bdata['poses'][fId:fId+1, 66:156]).to(comp_device)    # (1, 90); controls the finger articulation
    ...
    pose_seq_np_n[..., 0] *= -1
    #print("pose_seq[0] ", pose_seq_np_n[0][0])
...
def uniform_skeleton(
    ...
    #print("source offset ", src_offset)
    # print(tgt_offset)
    '''Calculate Scale Ratio as the ratio of legs'''
    src_leg_len = np.abs(src_offset[l_idx1]).max() + np.abs(src_offset[l_idx2]).max()
    tgt_leg_len = np.abs(tgt_offset[l_idx1]).max() + np.abs(tgt_offset[l_idx2]).max()
    #print("src leg len ", src_leg_len)
    #print("tgt leg len ", tgt_leg_len)
    scale_rt = tgt_leg_len / src_leg_len
    # print(scale_rt)
    src_root_pos = positions[:, 0]
    print("src root ", src_root_pos.shape)
    tgt_root_pos = src_root_pos * scale_rt

    '''Inverse Kinematics'''
    quat_params = src_skel.inverse_kinematics_np(positions, face_joint_indx)
    # print(quat_params.shape)
    #print("qps 1 ", quat_params)
...
def process_file(
    ...
    '''Uniform Skeleton'''
    #print("positions[10] ", positions[10])
    ...
    #print("root quat init ", root_quat_init)
    # positions_b = positions.copy()
    positions = qrot_np(root_quat_init, positions) # (124, 22, 3), before and after
    #print("rotated positions shape ", positions.shape)
    ...
    #print("feet_l_x ", feet_l_x.shape)
    feet_l_y = (positions[1:, fid_l, 1] - positions[:-1, fid_l, 1]) ** 2
    feet_l_z = (positions[1:, fid_l, 2] - positions[:-1, fid_l, 2]) ** 2
    feet_l = ((feet_l_x + feet_l_y + feet_l_z) < velfactor).astype(np.float32) # (123,2), binary array
    #print("feet_l ", feet_l)
    ...
    def get_cont6d_params(positions):
        ...
        #print("qps 2 ", quat_params)

        '''Quaternion to continuous 6D'''
        cont_6d_params = quaternion_to_cont6d_np(quat_params)
        # (seq_len, 4)
        r_rot = quat_params[:, 0].copy()
        #print("r_rot ", r_rot.shape)
        #print("r_rot ", r_rot)
    ...
    # Get Rifke
    print("r_rot shape ", r_rot.shape)
    #print("r_rot ", r_rot)
    print("cont_6d_params ", cont_6d_params.shape)
    def get_rifke(positions):
        '''Local pose'''
        positions[..., 0] -= positions[:, 0:1, 0]
        positions[..., 2] -= positions[:, 0:1, 2]
        '''All pose face Z+'''
        #print("rotation rifke ", np.repeat(r_rot[:, None], positions.shape[1], axis=1))
        #print("pre rotated pos[10] ", positions[10])
        positions = qrot_np(np.repeat(r_rot[:, None], positions.shape[1], axis=1), positions)
        return positions
    positions = get_rifke(positions) # (124, 22, 3) ---- can recover this
    #print("rifke positions ", positions[10])
    #print("positions[1] ", positions[1])
    #sprint("positions[2] ", positions[2])
    skel = Skeleton(n_raw_offsets, kinematic_chain, "cpu")
    rec_quat = skel.inverse_kinematics_np(positions, face_joint_indx, smooth_forward=True)[:, 0]
    print("rec_quat shape ", rec_quat.shape)
    #print("rec_quat ", rec_quat)
    '''Root height'''
    ...
    print("local vel shape ", local_vel.shape)
...
def motion_representation(pose_seq):
    ...
    example_id = "000021"
    example_data = np.load(os.path.join("./translation/utils_mdm", example_id + '.npy')) # (180, 52, 3)
    example_data = torch.from_numpy(example_data)
    tgt_skel = Skeleton(n_raw_offsets, kinematic_chain, 'cpu')
    tgt_offsets = tgt_skel.get_offsets_joints(example_data[0]) # (22, 3)
    #print("target offsets ", tgt_offsets)
    ...
    print("rec data shape ", rec_ric_data.shape)
    #print("rec data[0] ", rec_ric_data[0])
    #print("rec data[1] ", rec_ric_data[1])

    return data, rec_ric_data, ground_positions
...
def amass_to_mdm(amass_raw):
    ...
    pose_seq = raw_pose_processing(amass_raw) # (124, 52, 3)
    print("pose seq shape ", pose_seq.shape)
    pose_seq, pose_seq_3D, ground_positions = motion_representation(pose_seq)

    return pose_seq, pose_seq_3D, ground_positions
...
if __name__ == "__main__":
    ...
    #amass_raw_path = "data/amass/CMU/75/10_03_poses.npz"
    #amass_raw_path = "data/amass/CMU/75/accad25.npz"
    amass_raw = np.load(amass_raw_path)
    _, _, gr = amass_to_mdm(amass_raw)
    gr = np.array([gr[i] for i in range(0,gr.shape[0],3)])

    amass_raw_path = "np.load('data/mdm/75/75_09/sample00_rep00.npz')"
    amass_raw = np.load(amass_raw_path)
    _, _, gr_rec = amass_to_mdm(amass_raw)
