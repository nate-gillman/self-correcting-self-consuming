...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        # could be _WrappedModel where its .model property is MDM.
    ...
    def p_sample_loop(
        ...
        randomize_class: bool = False,
        cond_fn_with_grad: Optional[Callable] = False,
        ...
        :param dump_steps: list of step numbers to return, e.g. [1, 5, 10]
            returns the 1st denoising iteration, 5th, and 10th, etc.
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with torch.no_grad():
                ...
                yield out
                img = out["sample"]  # does this line run?
    ...
    def p_sample(
        ...
        cond_fn: Optional[Callable] = None,
        model_kwargs: Optional[Dict] = None,
        const_noise: Optional[bool] = False,
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        # cond_fn is false during typical training
    ...
    def plms_sample(
        ...
        if order > 1 and old_out is None:
            ...
            mean_pred = (
                pred_prime * torch.sqrt(alpha_bar_prev)
                + torch.sqrt(1 - alpha_bar_prev) * eps_prime
            )
        else:
    ...
    def velocity_consistency_loss_humanml3d(self, target, model_output):
        ...
        r_rot_quat, r_pos = motion_process.recover_root_rot_pos(
            target.permute(0, 2, 3, 1).type(torch.FloatTensor)
        )
