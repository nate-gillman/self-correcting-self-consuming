"""HumanML3D Dataset Helpers.

This code originally from: https://github.com/EricGuo5513/HumanML3D/blob/main/motion_representation.ipynb
...
def get_cont6d_params(positions):
    """Continuous 6D Represenation for Neural Networks.

    See: https://arxiv.org/abs/1812.07035
    Zhou, et al. "On the Continuity of Rotation Representations in Neural Networks" (2018)
    """
    skel = Skeleton(n_raw_offsets, kinematic_chain, "cpu")
    # (seq_len, joints_num, 4)
    quat_params = skel.inverse_kinematics_np(
...
def extract_features(
    ...
    """
    This function is based on 'process_file' from: https://github.com/EricGuo5513/HumanML3D/blob/main/motion_representation.ipynb
    """
...
def recover_from_ric(data: torch.Tensor, joints_num: int) -> torch.Tensor:
    """
    data is a tensor of shape: (batch_size, 1, seq_length, pose_dim)
        ex: (64, 1, 196, 263)

        one can obtain this data directly from the diffusion model sample_loop function,
        see: diffusion/gaussian_diffusion.py -> p_sample_loop()
        and then transposing the dimensions like this with:
        (batch_size, pose_dim, 1, seq_length) -> (batch_size, 1, seq_length, pose_dim), i.e.

            sample = sample.permute(0, 2, 3, 1)

        see sample/generate.py for an example.

    this returns a tensor of shape: (batch_size, 1, seq_length, num_joints, 3)

    RIC = Rotation Invariant C,
        where I am not quite sure what the 'C' is
