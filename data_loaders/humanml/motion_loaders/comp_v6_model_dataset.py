...
from sample.types import T_IDX_COLLATED_DIFFUSION_SAMPLE
...
class CompMDMGeneratedDataset(Dataset):
    def __init__(
        ...
        print(
            f"CompMDMGeneratedDataset: generate {num_samples_limit} samples in {real_num_batches} batche(s), with mm_idxs: {mm_idxs}"
        )
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(dataloader)):
                batch: T_IDX_COLLATED_DIFFUSION_SAMPLE
                motion, model_kwargs, _ = batch
                if (
                ...
                # this procedure can be quite slow during eval
                for e_i, t in enumerate(range(repeat_times)):
                    print(f"Repeat time: {e_i + 1}/{repeat_times}")
                    sample = sample_fn(
                        model,
                        motion.shape,
