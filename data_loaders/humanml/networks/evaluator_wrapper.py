...
class EvaluatorMDMWrapper(object):
    ...
    def get_co_embeddings(self, word_embs, pos_ohot, cap_lens, motions, m_lens):
        with torch.no_grad():
            ...
            try:
                ...
                movements = self.movement_encoder(motions[..., :-4].type(torch.float64)).detach()
            m_lens = m_lens // self.opt["unit_length"]
            motion_embedding = self.motion_encoder(movements, m_lens)
            ...
            try:
                ...
                text_embedding = self.text_encoder(word_embs.type(torch.float64), pos_ohot.type(torch.float64), cap_lens)
            text_embedding = text_embedding[align_idx]
        return text_embedding, motion_embedding
    ...
    def get_motion_embeddings(self, motions, m_lens):
        with torch.no_grad():
            ...
            """Movement Encoding"""
            try:
                movements = self.movement_encoder(motions[..., :-4]).detach() # (32, 196, 259) --> (32, 49, 512)
            except:
                movements = self.movement_encoder(motions[..., :-4].type(torch.float64)).detach() # (32, 196, 259) --> (32, 49, 512)

            m_lens = m_lens // self.opt["unit_length"]
            try:
                motion_embedding = self.motion_encoder(movements, m_lens) # (32, 512)
            except:
                motion_embedding = self.motion_encoder(movements.type(torch.float64), m_lens) # (32, 512)
