...
def evaluate(args, model, diffusion, data):
    ...
    if args.dataset == "humanact12":
        from eval.a2m.gru_eval import evaluate as evaluate_impl

        # EVALUATE HUMANACT 12
        eval_results = evaluate_impl(args, model, diffusion, data)
    elif args.dataset == "uestc":
        from eval.a2m.stgcn_eval import evaluate as evaluate_impl
...
def main():
    ...
    assert args.eval_mode in [
        "debug",
        "full",
    ], f"eval_mode {args.eval_mode} is not supported for dataset {args.dataset}"
